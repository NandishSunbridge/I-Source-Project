table 75112 "PR RFQ Line"
{
    Caption = 'PR RFQ Line';
    DrillDownPageID = "PR RFQ SubForm";
    LookupPageID = "PR RFQ SubForm";
    ...
    fields
    {
        field(50001; "Document No."; Code[20])
        {
            ...
            TableRelation = "PR RFQ Header"."Document No.";
            Caption = 'Document No.';
        }
            ...
            begin
                case Type of
                    ...
                    Type::Item:
                        if Item.Get("Item No.") then begin
                            ...
                            //"HSN/SAC" := Item."HSN/SAC Code";//AA 07Jun23
                ...
                if PurchInvLine.FindLast() then begin
                    repeat
                        ...
                        "Last Rate" := PurchInvLine."Direct Unit Cost";  // Last Purchase Price. changed by AK Jain on 25/07/12
                    ...
                    //AKS Start
                    PRRFQHeader.Reset();
                    PRRFQHeader.SetRange(PRRFQHeader."Document No.", "Document No.");
                    if PRRFQHeader.Find('-') then
                        DocDate := PRRFQHeader."Document Date";
    ...
    trigger OnInsert()
    begin
        if ("Document Type" <> PRRFQHeader."Document Type") or ("Document No." <> PRRFQHeader."Document No.") then
            PRRFQHeader.Get("Document Type", "Document No.");

        "Document Type" := "document type"::Indent;
        PRRFQHeader.TestField(Authorized, false);
        PRRFQHeader.TestField("Sent for Authorization", false);
    end;
    ...
    begin
        ...
        PRRFQHeader.Reset();
        PRRFQHeader.SetRange(PRRFQHeader."Document No.", "Document No.");
        if PRRFQHeader.Find('-') then begin
            "Document Type" := PRRFQHeader."Document Type";
            "Requisition Type" := PRRFQHeader."Requisition Type";
            "Requirement Type" := PRRFQHeader."Requirement Type";
            "Dept. Code" := PRRFQHeader."Dept. Code";
            "Shortcut Dimension 1 Code" := PRRFQHeader."Shortcut Dimension 1 Code";
            "Shortcut Dimension 2 Code" := PRRFQHeader."Shortcut Dimension 2 Code";
            "Location Code" := PRRFQHeader."Location Code";
            "Purchase Order" := PRRFQHeader."Purchase Order";  // Pass on to line
#pragma warning disable AA0206
            FixedAssest := false;  // Set Fixed Assest
#pragma warning restore AA0206
            if PRRFQHeader."Fixed Assets Code" <> '' then
                FixedAssest := true;
...
#pragma warning restore AA0217
   ...
   #pragma warning restore AL0432
               ...
               PRRFQHeader."Dimension Set ID", Database::Item);
           DimensionManagement.UpdateGlobalDimFromDimSetID("Dimension Set ID", "Shortcut Dimension 1 Code", "Shortcut Dimension 2 Code");*/
    end;
